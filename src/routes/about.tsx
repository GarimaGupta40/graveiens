    ...
    <>
        ...
        <div className="grid gap-14 lg:grid-cols-2 lg:items-center">
          ...
          <FadeUp delay={0.1}>
            <div className="relative">
              <div className="absolute -inset-6 rounded-3xl bg-gradient-to-br from-brand/20 via-transparent to-accent-teal/20 blur-2xl" />
              <div className="relative grid gap-4 rounded-3xl border border-hairline bg-white p-6 shadow-elegant">
                {[
                  { year: "2015", title: "Founded", desc: "Started with a mission to modernize digital publishing." },
                  { year: "2018", title: "Global expansion", desc: "Scaled operations across North America, EMEA and APAC." },
                  { year: "2021", title: "AI-first authoring", desc: "Launched our internal AI content pipeline." },
                  { year: "2024", title: "Enterprise learning", desc: "Expanded into corporate L&D and government programs." },
                ].map((m, i) => (
                  <div
                    key={m.year}
                    className="flex gap-4 rounded-2xl border border-hairline bg-surface-alt p-4"
                  >
                    <div className="grid h-12 w-16 shrink-0 place-items-center rounded-xl bg-gradient-brand font-display text-sm font-bold text-white">
                      {m.year}
                    </div>
                    <div>
                      ...
                      <div className="mt-1 text-sm text-ink-muted">{m.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="relative bg-surface-alt py-24 lg:py-32">
        ...
        <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
          <SectionHeader
            ...
            title={<>Purpose that shapes every <span className="text-gradient-brand">decision</span></>}
            description="Three principles guide every engagement — from a single storyboard to a multi-year program."
          />
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              ...
              <FadeUp key={v.title} delay={i * 0.06}>
                <div className="group h-full rounded-3xl border border-hairline bg-white p-8 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-hover">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-ink-muted">{v.desc}</p>
                </div>
              </FadeUp>
            ))}
          </div>
      ...
      <Section className="bg-surface-blue">
        <SectionHeader
          ...
          title={<>Delivering across <span className="text-gradient-brand">four continents</span></>}
          description="Regional teams and studios that combine local craft with global delivery discipline."
        />
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <FadeUp key={o.region} delay={i * 0.05}>
              <div className="h-full rounded-2xl border border-hairline bg-white p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-hover">
                ...
                <div className="mt-2 text-sm text-ink-muted">{o.role}</div>
              </div>
            </FadeUp>
          ))}
        </div>
